  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-center gap-3 overflow-hidden whitespace-nowrap rounded-full border border-lime-300/15 bg-black/35 px-5 py-3 shadow-2xl backdrop-blur-2xl">
        <span className={`shrink-0 rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[0.22em] ${role === 'user' ? 'border border-sky-400/20 bg-sky-500/10 text-sky-300' : 'border border-lime-300/25 bg-lime-400/10 text-lime-300'}`}>
          {role === 'user' ? 'You' : name}
        </span>
...
function LimeVoiceOrb({ isActive, isAgentSpeaking, speakerLevel, speakerBands }: { isActive: boolean; isAgentSpeaking: boolean; speakerLevel: number; speakerBands: number[]; }) {
  ...
  useEffect(() => {
    levelRef.current = speakerLevel; bandsRef.current = speakerBands; activeRef.current = isActive; speakingRef.current = isAgentSpeaking;
  ...
  useEffect(() => {
    ...
    let frame = 0; let raf = 0; let displayLevel = 0;
    ...
    const makeOrbPath = (cx: number, cy: number, radius: number, pulse: number, time: number) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const surface = Math.sin(angle * 2.1 + time * 0.95) * (live ? 2.5 : 0.9) + Math.sin(angle * 3.7 - time * 0.68) * (live ? 1.7 : 0.55) + band * (live ? 8.5 : 1.8);
      ...
      points.forEach((point, index) => {
        ...
        if (index === 0) path.moveTo(midX, midY); else path.quadraticCurveTo(point.x, point.y, midX, midY);
      });
      path.closePath();
      return path;
    };
    ...
    const draw = () => {
      ...
      const cx = width / 2; const cy = height / 2; const time = frame / 60;
      ...
      ctx.save(); ctx.globalAlpha = activeRef.current ? 0.42 + pulse * 0.28 : 0.24; ctx.filter = 'blur(34px)';
      drawGlow(cx, cy, 118 + pulse * 22, 'rgba(190,242,100,0.42)', 'rgba(22,101,52,0)'); ctx.restore();

      ctx.save(); ctx.globalAlpha = activeRef.current ? 0.24 + pulse * 0.26 : 0.12; ctx.strokeStyle = 'rgba(190,242,100,0.34)'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(cx, cy, 116 + pulse * 16, 0, Math.PI * 2); ctx.stroke(); ctx.restore();

      const orbPath = makeOrbPath(cx, cy, baseRadius, pulse, time);

      ctx.save(); ctx.shadowColor = 'rgba(190,242,100,0.38)'; ctx.shadowBlur = 38 + pulse * 28;
      const bodyGradient = ctx.createRadialGradient(cx - 38, cy - 48, 8, cx, cy, 126);
      bodyGradient.addColorStop(0, 'rgba(236,252,203,0.76)'); bodyGradient.addColorStop(0.27, 'rgba(163,230,53,0.58)');
      bodyGradient.addColorStop(0.58, 'rgba(34,197,94,0.46)'); bodyGradient.addColorStop(1, 'rgba(5,46,22,0.96)');
      ctx.fillStyle = bodyGradient; ctx.fill(orbPath); ctx.restore();

      ctx.save(); ctx.clip(orbPath); ctx.globalCompositeOperation = 'screen';
      ...
      ctx.save(); ctx.globalAlpha = live ? 0.14 + pulse * 0.18 : 0.06; ctx.fillStyle = 'rgba(255,255,255,0.58)';
      ctx.beginPath(); ctx.ellipse(cx - 36, cy - 46, 24 + pulse * 6, 11 + pulse * 3, -0.55, 0, Math.PI * 2); ctx.fill(); ctx.restore();
...
// -------------------------------------------------------------
function MeetingRecorderModal({ onClose, onProcess }: { onClose: () => void, onProcess: (transcript: string) => Promise<void> }) {
      ...
      <div className="w-full h-full flex flex-col max-w-5xl items-center justify-center">
        {isProcessing ? (
           <div className="flex flex-col items-center py-20 text-center">
             ...
             <p className="text-zinc-500 text-lg max-w-lg">Beatrice is reviewing the transcript, generating formal minutes, and extracting tasks to update your schedule.</p>
           </div>
        ) : (
           <>
             <div className="flex justify-between items-center w-full mb-12">
               <h2 className="text-2xl font-bold text-white uppercase tracking-widest">Meeting Recorder</h2>
               <button onClick={handleCancel} className="text-zinc-500 hover:text-white transition"><X className="w-8 h-8" /></button>
             </div>
               ...
               <div className="relative flex items-center justify-center w-48 h-48 mb-10">
                 ...
                 <button
                   ...
                   className={`relative z-10 w-32 h-32 rounded-full flex items-center justify-center transition-all ${
                     isRecording ? 'bg-red-500 hover:bg-red-600 shadow-[0_0_40px_rgba(239,68,68,0.6)]' : 'bg-lime-300 hover:bg-lime-400 text-black shadow-[0_0_40px_rgba(190,242,100,0.4)]'
                   }`}
                 >
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(fontId)) {
      ...
      link.id = fontId; link.rel = 'stylesheet'; link.href = 'https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700;900&display=swap';
      ...
      if (u) {
        try {
          ...
          if (!userSnap.exists()) {
            ...
            await set(userRef, { displayName: initialSettings.userName, email: u.email || '', authProvider, googleServicesConnected: hasGoogleServices, createdAt: serverTimestamp(), updatedAt: serverTimestamp(), settings: initialSettings });
            setSettings(initialSettings);
          } else {
            const data = userSnap.val();
            if (data.settings) setSettings({ ...DEFAULT_SETTINGS, ...data.settings });
            await update(userRef, { email: u.email || data.email || '', authProvider, googleServicesConnected: hasGoogleServices, updatedAt: serverTimestamp() });
          }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error && error.message && error.message.includes('missing initial state')) setAuthMessage({ type: 'error', text: "Authentication failed due to browser privacy settings. Open the app in a new tab and try again." });
      else setAuthMessage({ type: 'error', text: getAuthErrorMessage(error) });
    } finally { setAuthBusy(false); }
  };

  const handleEmailAuth = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setAuthBusy(true); setAuthMessage(null);
    const email = authEmail.trim(); const password = authPassword.trim(); const fullName = authName.trim();
    try {
      if (!email) throw new Error('Enter your email address.');
      if (authMode === 'reset') { await sendPasswordResetEmail(auth, email); setAuthMessage({ type: 'success', text: 'Password reset email sent. Check your inbox.' }); setAuthMode('signin'); return; }
      ...
      await signInWithEmailAndPassword(auth, email, password); localStorage.removeItem('googleAccessToken');
    } catch (error: any) { setAuthMessage({ type: 'error', text: getAuthErrorMessage(error) }); } finally { setAuthBusy(false); }
  };

  const handleLogout = () => { localStorage.removeItem('googleAccessToken'); signOut(auth); };

  if (loading) return ( <div className="flex min-h-screen items-center justify-center bg-[#020203] text-zinc-500"><div className="flex flex-col items-center gap-4"><Loader2 className="h-8 w-8 animate-spin" /><p className="animate-pulse text-[10px] uppercase tracking-widest">Preparing VEP...</p></div></div> );

  if (!user) {
    const isSignUp = authMode === 'signup'; const isReset = authMode === 'reset';
        ...
        <motion.main key={authMode} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, ease: 'easeOut' }} className="relative z-10 mx-auto flex min-h-[100dvh] w-full max-w-[430px] flex-col px-6 pb-[max(22px,env(safe-area-inset-bottom))] pt-[max(28px,env(safe-area-inset-top))]">
          <section className="flex flex-1 flex-col justify-center py-10">
            ...
            <form onSubmit={handleEmailAuth} className="space-y-3">
              {isSignUp && ( <label className="flex h-14 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 focus-within:border-lime-300/40"><UserRound className="h-4 w-4 shrink-0 text-zinc-500" /><input value={authName} onChange={(e) => setAuthName(e.target.value)} placeholder="Full name" autoComplete="name" className="min-w-0 flex-1 bg-transparent text-sm font-medium text-white outline-none placeholder:text-zinc-600" /></label> )}
              <label className="flex h-14 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 focus-within:border-lime-300/40"><Mail className="h-4 w-4 shrink-0 text-zinc-500" /><input value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} type="email" placeholder="Email" autoComplete="email" className="min-w-0 flex-1 bg-transparent text-sm font-medium text-white outline-none placeholder:text-zinc-600" /></label>
              {!isReset && ( <label className="flex h-14 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 focus-within:border-lime-300/40"><LockKeyhole className="h-4 w-4 shrink-0 text-zinc-500" /><input value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} type={showAuthPassword ? 'text' : 'password'} placeholder="Password" autoComplete={isSignUp ? 'new-password' : 'current-password'} className="min-w-0 flex-1 bg-transparent text-sm font-medium text-white outline-none placeholder:text-zinc-600" /><button type="button" onClick={() => setShowAuthPassword(v => !v)} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-zinc-500 transition hover:bg-white/5 hover:text-zinc-200">{showAuthPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>{!isSignUp && ( <button type="button" onClick={() => { setAuthMode('reset'); setAuthMessage(null); }} className="text-xs font-bold text-lime-200">Forgot?</button> )}</label> )}
              {isSignUp && ( <label className="flex h-14 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] px-4 focus-within:border-lime-300/40"><LockKeyhole className="h-4 w-4 shrink-0 text-zinc-500" /><input value={authConfirmPassword} onChange={(e) => setAuthConfirmPassword(e.target.value)} type={showAuthConfirmPassword ? 'text' : 'password'} placeholder="Confirm password" autoComplete="new-password" className="min-w-0 flex-1 bg-transparent text-sm font-medium text-white outline-none placeholder:text-zinc-600" /><button type="button" onClick={() => setShowAuthConfirmPassword(v => !v)} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-zinc-500 transition hover:bg-white/5 hover:text-zinc-200">{showAuthConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></label> )}
              {authMessage && ( <div className={`rounded-2xl px-4 py-3 text-xs leading-5 ${authMessage.type === 'error' ? 'border border-red-400/20 bg-red-500/10 text-red-200' : authMessage.type === 'success' ? 'border border-lime-300/20 bg-lime-300/10 text-lime-100' : 'border border-white/10 bg-white/[0.06] text-zinc-300'}`}>{authMessage.text}</div> )}
              <button type="submit" disabled={authBusy} className="mt-7 flex h-14 w-full items-center justify-center rounded-full bg-lime-300 text-sm font-bold text-black shadow-[0_18px_48px_rgba(190,242,100,0.18)] transition active:scale-[0.985] disabled:opacity-60">{authBusy ? <Loader2 className="h-5 w-5 animate-spin" /> : isReset ? 'Send reset link' : isSignUp ? 'Sign up' : 'Sign in'}</button>
              {!isReset && ( <><div className="flex items-center gap-3 py-1.5"><div className="h-px flex-1 bg-white/10" /><span className="text-xs font-medium text-zinc-600">or</span><div className="h-px flex-1 bg-white/10" /></div><button type="button" onClick={handleGoogleLogin} disabled={authBusy} className="flex h-14 w-full items-center justify-center gap-3 rounded-full border border-white/10 bg-white/[0.06] px-5 text-sm font-bold text-zinc-100 transition hover:border-lime-300/30 hover:bg-lime-300/10 active:scale-[0.985] disabled:opacity-60"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-white text-base font-black text-black">G</span>Continue with Google</button></> )}
            </form>
          </section>
          <footer className="text-center text-sm text-zinc-500">{isSignUp ? 'Back to ' : isReset ? 'Remembered it? ' : 'Create account? '}<button type="button" onClick={() => { setAuthMode(isSignUp || isReset ? 'signin' : 'signup'); setAuthMessage(null); }} className="font-bold text-lime-200">{isSignUp || isReset ? 'Sign in' : 'Sign up'}</button></footer>
        </motion.main>
      </div>
    );
  }
...
function BeatriceAgent({ user, onLogout, initialSettings }: { user: User; onLogout: () => void; initialSettings: AgentSettings; }) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(historyRef, (snap) => {
      const msgs: string[] =[]; const rawMsgs: ChatMessage[] =[];
      snap.forEach(child => { const m = child.val() as ChatMessage; msgs.push(`${m.role.toUpperCase()}: ${m.text}`); rawMsgs.push(m); });
  ...
  const selectedVoiceMeta = useMemo(() => GEMINI_LIVE_VOICE_OPTIONS.find(v => v.id === settings.selectedVoice) || GEMINI_LIVE_VOICE_OPTIONS[0],[settings.selectedVoice]);

  const saveMessage = (role: 'user' | 'model', text: string, extra?: Partial<ChatMessage>) => {
    const clean = text.trim(); if (!clean) return;
    try { const msgRef = push(ref(rtdb, 'users/' + user.uid + '/messages')); set(msgRef, { role, text: clean, timestamp: Date.now(), ...extra }); } catch (e) { console.error(e); }
  };
  ...
  const saveModelBuffer = () => {
    ...
    lastSavedModelTranscriptRef.current = clean; saveMessage('model', clean); modelTranscriptBufferRef.current = '';
  };
  ...
  const saveUserBuffer = () => {
    ...
    lastSavedUserTranscriptRef.current = clean; saveMessage('user', clean); userTranscriptBufferRef.current = '';
  };
  ...
  const startMicVisualizer = () => {
    const tick = () => {
      const recorder: any = audioRecorderRef.current; const streamer: any = audioStreamerRef.current;
      let nextLevel = 0; let nextBands = Array(20).fill(0); let nextSpeakerLevel = 0; let nextSpeakerBands = Array(20).fill(0);
      try {
        if (recorder && typeof recorder.getFrequencyBands === 'function') {
          const bands = recorder.getFrequencyBands(20) ||[]; nextBands = bands.map((n: number) => Math.min(1, Math.max(0, Number(n || 0))));
          nextLevel = Math.min(1, Math.max(typeof recorder.getLevel === 'function' ? recorder.getLevel() : 0, (nextBands.reduce((sum: number, n: number) => sum + n, 0) / Math.max(nextBands.length, 1)) * 1.8));
        } else if (isActiveRef.current && !isMutedRef.current) { nextLevel = 0.06; nextBands = Array(20).fill(0.04); }
      } catch (e) { nextLevel = 0; nextBands = Array(20).fill(0); }
      try {
        if (streamer && typeof streamer.getFrequencyBands === 'function') {
          const bands = streamer.getFrequencyBands(20) ||[]; nextSpeakerBands = bands.map((n: number) => Math.min(1, Math.max(0, Number(n || 0))));
          nextSpeakerLevel = Math.min(1, Math.max(typeof streamer.getLevel === 'function' ? streamer.getLevel() : 0, (nextSpeakerBands.reduce((sum: number, n: number) => sum + n, 0) / Math.max(nextSpeakerBands.length, 1)) * 1.65));
        }
      ...
      setMicLevel(prev => prev + (nextLevel - prev) * 0.46); setMicBands(prev => nextBands.map((band: number, i: number) => prev[i] + (band - prev[i]) * 0.42));
      setSpeakerLevel(prev => prev + (nextSpeakerLevel - prev) * 0.5); setSpeakerBands(prev => nextSpeakerBands.map((band: number, i: number) => prev[i] + (band - prev[i]) * 0.48));
  ...
  const stopMicVisualizer = () => {
    if (micAnimationFrameRef.current) cancelAnimationFrame(micAnimationFrameRef.current);
    micAnimationFrameRef.current = null; setMicLevel(0); setMicBands(Array(20).fill(0)); setSpeakerLevel(0); setSpeakerBands(Array(20).fill(0));
  };
  ...
  const sendChatMessage = (e?: FormEvent) => {
    ...
    const clean = chatInput.trim(); if (!clean) return;
  ...
  const createGoogleDoc = async (title: string, content: string) => {
    ...
    if (content?.trim()) await googleJson(`https://docs.googleapis.com/v1/documents/${doc.documentId}:batchUpdate`, { method: 'POST', body: JSON.stringify({ requests:[{ insertText: { location: { index: 1 }, text: content } }] }) });
  ...
  const uploadTextFileToDrive = async (fileName: string, content: string, mimeType = 'text/plain', folderId?: string) => {
    const metadata: any = { name: fileName }; if (folderId) metadata.parents =[folderId];
    const boundary = `boundary_${Date.now()}`;
    const multipartBody = `--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${JSON.stringify(metadata)}\r\n--${boundary}\r\nContent-Type: ${mimeType}\r\n\r\n${content || ''}\r\n--${boundary}--`;
    return googleFetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,mimeType,webViewLink,webContentLink', { method: 'POST', headers: { 'Content-Type': `multipart/related; boundary=${boundary}` }, body: multipartBody }).then(r => r.json());
  };
  ...
  const executeGoogleTool = async (toolName: string, args: any) => {
    ...
    const handleDocumentGeneration = async (html: string, defaultTitle: string) => {
        ...
        let driveFile: any = null; let emailResult: any = null;
        ...
        if (emailTo) {
            emailResult = await sendGmail({
                to: emailTo, subject: title, body: `Attached is the generated document: ${title}. Please open the HTML file in your browser to view or print as PDF.`,
                attachment: { filename: htmlFile.htmlPreviewFilename, mimeType: 'text/html', base64Content: btoa(unescape(encodeURIComponent(htmlFile.html))) },
            });
        }
        return { toolName, executedAt, status: 'completed', title, driveLink: driveFile?.webViewLink, emailSentTo: emailTo || null, emailResult, summary: `Generated and saved document: ${title}`, ...htmlFile };
    };

    try {
      switch (toolName) {
        case 'create_invoice_document': return await handleDocumentGeneration(buildInvoiceHtml(args), 'Invoice');
        case 'create_meeting_minutes': return await handleDocumentGeneration(buildMeetingMinutesHtml(args), 'Meeting Minutes');
        case 'generate_data_dashboard': return await handleDocumentGeneration(buildDashboardHtml(args), 'Data Dashboard');
        case 'generate_project_gantt_chart': return await handleDocumentGeneration(buildGanttHtml(args), 'Project Timeline');
        case 'create_contract_document': return await handleDocumentGeneration(buildStunningHtmlContract(args), `${args.contractType || 'Contract'} - ${args.partyA || 'Party A'} and ${args.partyB || 'Party B'}`);
        ...
        case 'render_web_artifact': {
          ...
          if (emailTo) emailResult = await sendGmail({ to: emailTo, subject: title, body: `Attached is the standalone HTML artifact.`, attachment: { filename: htmlFile.htmlPreviewFilename, mimeType: 'text/html', base64Content: btoa(unescape(encodeURIComponent(htmlFile.html))) } });
          return { toolName, executedAt, status: 'completed', title, note: args?.summary || 'Created HTML artifact.', driveFile, emailSentTo: emailTo || null, emailResult, ...htmlFile };
        }

        case 'gmail_read': {
          const queryText = args?.query || ''; const limit = Math.min(Number(args?.limit || 10), 20);
          ...
          const messages = await Promise.all((list.messages ||[]).map(async (m: any) => {
              ...
              const headers = msg.payload?.headers ||[]; const findHeader = (name: string) => headers.find((h: any) => h.name?.toLowerCase() === name.toLowerCase())?.value || '';
        ...
        case 'drive_search': {
          const q = args.query || ''; const limit = Math.min(Number(args.limit || 10), 50); const escaped = q.replace(/'/g, "\\'"); let mimeClause = '';
          if (args.fileType) {
        ...
        case 'drive_read_file': {
          ...
          if (meta.mimeType?.startsWith('application/vnd.google-apps') && exportMimeType) {
            const blob = await exportDriveFile(fileId, exportMimeType); const text = exportMimeType.startsWith('text/') ? await blob.text() : ''; const downloadData = await makeBlobDownloadData(blob);
          ...
          const res = await googleFetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`); const blob = await res.blob(); const downloadData = await makeBlobDownloadData(blob);
        ...
        case 'workspace_search': {
          ...
          if (sources.includes('mail') || sources.includes('gmail')) { try { output.mail = await executeGoogleTool('gmail_read', { query: args.query, limit: 5 }); } catch (e: any) { output.mail = { error: e.message }; } }
          if (sources.includes('drive') || sources.includes('files')) { try { output.drive = await executeGoogleTool('drive_search', { query: args.query, limit: 5 }); } catch (e: any) { output.drive = { error: e.message }; } }
          if (sources.includes('calendar')) { try { output.calendar = await executeGoogleTool('calendar_check_schedule', { date: new Date().toISOString() }); } catch (e: any) { output.calendar = { error: e.message }; } }
  ...
  const startSession = async () => {
    if (!aiRef.current) { alert('Gemini API key is missing. Add VITE_GEMINI_API_KEY.'); return; }
    setConnecting(true); modelTranscriptBufferRef.current = ''; userTranscriptBufferRef.current = '';
    ...
    try {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            if (msg.toolCall && msg.toolCall.functionCalls) {
                ...
                for (const c of msg.toolCall.functionCalls) {
                  const toolName = c.name || 'unknown_tool'; const args = c.args as any; const tid = Math.random().toString(36).substring(7);
                  setTasks(p =>[...p, { id: tid, serviceName: toolName, action: safeJsonStringify(args || {}), status: 'processing' }]);
                  try {
                ...
                if (resps.length > 0 && sessionRef.current && typeof sessionRef.current.sendToolResponse === 'function') sessionRef.current.sendToolResponse({ functionResponses: resps });
            }
            if (msg.serverContent) {
              const serverContent: any = msg.serverContent;
              if (serverContent.interrupted) { audioStreamerRef.current?.stop(); setIsAgentSpeaking(false); modelTranscriptBufferRef.current = ''; return; }
              if (serverContent.inputTranscription?.text) { userTranscriptBufferRef.current = serverContent.inputTranscription.text.trim(); updateLiveTranscript('user', userTranscriptBufferRef.current, 3200); }
              if (serverContent.outputTranscription?.text) { modelTranscriptBufferRef.current = (modelTranscriptBufferRef.current + serverContent.outputTranscription.text).trim(); updateLiveTranscript('model', modelTranscriptBufferRef.current, 3900); }
              if (serverContent.modelTurn?.parts) {
                for (const part of serverContent.modelTurn.parts) {
                  if (part.inlineData?.data) { audioStreamerRef.current?.addPCM16(part.inlineData.data); setIsAgentSpeaking(true); setTimeout(() => setIsAgentSpeaking(false), 620); }
                  if (part.text?.trim()) { modelTranscriptBufferRef.current = (modelTranscriptBufferRef.current + ' ' + part.text).trim(); updateLiveTranscript('model', modelTranscriptBufferRef.current, 3900); }
                }
              }
              if (serverContent.turnComplete) { saveModelBuffer(); saveUserBuffer(); }
            }
          },
          onclose: () => stopSession(),
      ...
      setIsActive(true); isActiveRef.current = true; setConnecting(false); startMicVisualizer();
      setTimeout(() => { sendTextToLive(`${settings.userName} is here in the office. Start like ${settings.agentName} is already sitting at the desk nearby as the office employee. Begin in English normally and respectfully.`); }, 500);
  ...
  const toggleVideo = async () => {
    if (!isVideoEnabled) {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode, width: 1280, height: 720 }, audio: false });
        if (videoRef.current) { videoRef.current.srcObject = stream; await videoRef.current.play(); }
        setIsVideoEnabled(true);
        setTimeout(() => { sendTextToLive(`${settings.userName} just opened the camera. Notice it in a normal human way. Oh, yeah, I see it now. Briefly describe only what is actually visible.`); }, 300);
      ...
      if (videoRef.current && videoRef.current.srcObject) { const stream = videoRef.current.srcObject as MediaStream; stream.getTracks().forEach(t => t.stop()); videoRef.current.srcObject = null; }
  ...
  const capturePhoto = () => {
    if (sessionRef.current && videoRef.current && canvasRef.current) {
      ...
      if (ctx && v.videoWidth && v.videoHeight) {
        ...
        if (base64Data) { sendTextToLive(`${settings.userName} captured this photo. Look at it and respond normally, briefly, and clearly.`); sendVideoToLive(base64Data); saveMessage('user', '[Sent Photo]'); }
      }
    }
  };
  ...
  const switchCamera = async () => {
    ...
    if (isVideoEnabled) {
      if (videoRef.current && videoRef.current.srcObject) { const stream = videoRef.current.srcObject as MediaStream; stream.getTracks().forEach(t => t.stop()); }
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: newMode, width: 1280, height: 720 }, audio: false });
        if (videoRef.current) { videoRef.current.srcObject = stream; videoRef.current.play().catch(e => console.error('Video play err', e)); }
  ...
  const stopSession = () => {
    try { recognitionRef.current?.stop(); } catch (e) {} try { audioRecorderRef.current?.stop(); } catch (e) {} try { audioStreamerRef.current?.stop(); } catch (e) {} try { sessionRef.current?.close(); } catch (e) {}
    stopMicVisualizer();
    if (videoIntervalRef.current) clearInterval(videoIntervalRef.current);
    if (videoRef.current && videoRef.current.srcObject) { const stream = videoRef.current.srcObject as MediaStream; stream.getTracks().forEach(t => t.stop()); videoRef.current.srcObject = null; }
    sessionRef.current = null; recognitionRef.current = null; modelTranscriptBufferRef.current = ''; userTranscriptBufferRef.current = ''; isActiveRef.current = false;
...
3. Return a brief conversational summary of what you did.`;
        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
            for (const call of response.functionCalls) {
                ...
                saveMessage('model', result.summary || `Completed Task: ${call.name}`, { toolName: call.name, toolResult: result, ...download });
            }
  ...
  return (
    <div className="relative flex h-[100dvh] min-h-screen flex-col overflow-hidden bg-[#020203] text-zinc-300 selection:bg-lime-300/30" style={{ fontFamily: 'Roboto, system-ui, sans-serif' }}>
      ...
      <AnimatePresence>
        {showMeetingRecorder && (
          <MeetingRecorderModal onClose={() => setShowMeetingRecorder(false)} onProcess={handleProcessMeeting} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isVideoEnabled && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 bg-black">
            ...
            <div className="absolute left-6 top-6 flex items-center gap-2 rounded-full border border-lime-300/20 bg-black/60 px-3 py-1.5 backdrop-blur-md"><span className="h-2 w-2 animate-pulse rounded-full bg-lime-300 shadow-[0_0_8px_rgba(190,242,100,0.9)]" /><span className="text-[9px] font-bold uppercase tracking-widest text-lime-200">Camera Live</span></div>
            ...
            <AnimatePresence>
              {currentTranscript && ( <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="pointer-events-none absolute left-1/2 top-[106px] z-50 w-[92vw] max-w-5xl -translate-x-1/2"><OneLineStreamingTranscript role={currentTranscript.role} text={currentTranscript.text} name={settings.agentName} /></motion.div> )}
            </AnimatePresence>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-8 z-50 flex flex-col items-center justify-end">
            ...
            <div className="pointer-events-auto flex flex-col items-center justify-center gap-4">
              <div className="flex items-center justify-center gap-8">
                <button onClick={() => setIsMuted(p => !p)} className={`flex h-12 w-12 items-center justify-center rounded-full border shadow-lg transition-all ${isMuted ? 'border-red-500/30 bg-red-500/10 text-red-500' : 'border-white/10 bg-[#0A0A0B] text-zinc-400 hover:border-white/30 hover:text-white'}`}>{isMuted ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}</button>
                {!isActive ? ( <StartIconMicVisualizer isActive={false} connecting={connecting} isMuted={isMuted} micLevel={0} micBands={micBands} onClick={startSession} /> ) : ( <StartIconMicVisualizer isActive={true} connecting={connecting} isMuted={isMuted} micLevel={micLevel} micBands={micBands} onClick={stopSession} /> )}
                <button onClick={() => toggleVideo()} className={`flex h-12 w-12 items-center justify-center rounded-full border shadow-lg transition-all ${isVideoEnabled ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-500' : 'border-white/10 bg-[#0A0A0B] text-zinc-400 hover:border-white/30 hover:text-white'}`}>{isVideoEnabled ? <Video className="h-5 w-5" /> : <VideoOff className="h-5 w-5" />}</button>
              </div>
            </div>
          </div>
        </main>
          ...
          <>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed bottom-0 left-0 top-0 z-[101] flex w-96 max-w-[88vw] flex-col border-r border-white/10 bg-[#0A0A0B] shadow-2xl">
              ...
              <div className="grid grid-cols-2 gap-3 border-b border-white/10 p-4">
                ...
                <button onClick={() => { setShowSidebar(false); setShowMeetingRecorder(true); }} className="col-span-2 flex w-full items-center justify-center gap-2 rounded-2xl border border-sky-400/30 bg-sky-400/10 px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-sky-300 transition hover:bg-sky-400/20"><Mic className="h-4 w-4" /> Record Meeting & Analyze</button>
              </div>
                ...
                <div className="flex-1 space-y-3 overflow-y-auto p-4 pb-3">
                  {historyMsgs.map((msg, i) => (
                    <div key={`${msg.timestamp}-${i}`} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`max-w-[92%] rounded-2xl p-3 text-xs leading-relaxed overflow-hidden break-words whitespace-pre-wrap ${msg.role === 'user' ? 'rounded-tr-sm border border-sky-400/20 bg-sky-400/10 text-sky-100' : 'rounded-tl-sm border border-lime-300/10 bg-white/5 text-zinc-300'}`}>
                        
                        {msg.fileDataUrl && (
                          <div className="mb-2 flex w-full justify-center overflow-hidden rounded-xl border border-white/10 bg-black/40">
                            <img src={msg.fileDataUrl} alt="Preview" className="max-h-48 w-auto object-contain" />
                          </div>
                        )}
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 z-[200] flex flex-col overflow-y-auto bg-[#050505]">
            ...
            <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-8 p-6 pb-32">
              ...
              <div className="space-y-6">
                <div className="space-y-2"><label className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-500"><UserRound className="h-3.5 w-3.5" /> How should Beatrice address you?</label><input type="text" value={settings.userName} onChange={(e) => setSettings(s => ({ ...s, userName: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-[#0A0A0B] p-4 text-xl font-medium text-white outline-none transition-all focus:border-lime-300/50 focus:ring-1 focus:ring-lime-300/50" placeholder="e.g. Jo Lernout" /></div>
                <div className="space-y-2"><label className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-500"><Bot className="h-3.5 w-3.5" /> Persona Name</label><input type="text" value={settings.agentName} onChange={(e) => setSettings(s => ({ ...s, agentName: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-[#0A0A0B] p-4 text-xl font-medium text-white outline-none transition-all focus:border-lime-300/50 focus:ring-1 focus:ring-lime-300/50" placeholder="e.g. Beatrice" /></div>
                <div className="space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Voice Alias</label><select value={settings.selectedVoice} onChange={(e) => setSettings(s => ({ ...s, selectedVoice: e.target.value }))} className="w-full rounded-xl border border-white/10 bg-[#0A0A0B] p-4 text-sm text-white outline-none transition-all focus:border-lime-300/50 focus:ring-1 focus:ring-lime-300/50">{GEMINI_LIVE_VOICE_OPTIONS.map(v => ( <option key={v.id} value={v.id}>{v.alias} — {v.vibe}</option> ))}</select></div>
                <div className="flex flex-1 flex-col space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Default Persona Instructions</label><textarea value={settings.personality} onChange={(e) => setSettings(s => ({ ...s, personality: e.target.value }))} className="min-h-[340px] w-full resize-y rounded-xl border border-white/10 bg-[#0A0A0B] p-4 font-mono text-xs leading-relaxed text-zinc-300 outline-none transition-all focus:border-lime-300/50 focus:ring-1 focus:ring-lime-300/50" placeholder="Describe how the agent should behave..." /><p className="text-[10px] leading-relaxed text-zinc-600">The hidden office-behavior prompt stays applied behind this editable persona.</p></div>
              </div>
            </div>
            <div className="fixed bottom-0 left-0 right-0 z-[220] border-t border-white/10 bg-[#050505]/90 p-4 backdrop-blur-xl">
              <div className="mx-auto flex w-full max-w-2xl gap-3"><button onClick={onLogout} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-500/10 px-4 py-3 text-xs font-bold uppercase tracking-widest text-red-500 transition-all hover:bg-red-500/20 active:scale-95"><LogOut className="h-4 w-4" /> Logout</button><button onClick={persistSettings} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-lime-300 px-4 py-3 text-xs font-bold uppercase tracking-widest text-black transition-all hover:bg-lime-200 active:scale-95"><Save className="h-4 w-4" /> Save</button></div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
